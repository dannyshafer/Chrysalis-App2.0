  ...
  def make_industry_averages
    Industry.all.each do |ind|
      ind.update_attributes(
        eps: Stock.where("industry = ? AND eps >= ?", ind.name, 0).average(:eps),
        pe: Stock.where("industry = ? AND pe >= ?", ind.name, 0).average(:pe),
        pbook: Stock.where("industry = ? AND pbook >= ?", ind.name, 0).average(:pbook),
        psales: Stock.where("industry = ? AND psales >= ?", ind.name, 0).average(:psales),
        markcap: Stock.where("industry = ? AND markcap >= ?", ind.name, 0).average(:markcap),
        peg: Stock.where("industry = ? AND peg >= ?", ind.name, 0).average(:peg),
